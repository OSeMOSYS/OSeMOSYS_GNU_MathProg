...
mkdir fast
glpsol -m src/osemosys_fast.txt -d $TEST_CASE -o fast.sol --xcheck --nopresol
rm results/SelectedResults.csv
cp -f results/*.csv fast

mkdir short
glpsol -m src/osemosys_short.txt -d $TEST_CASE -o short.sol --xcheck --nopresol
rm results/SelectedResults.csv
cp -f results/*.csv short

mkdir long
glpsol -m src/osemosys.txt -d $TEST_CASE -o long.sol --xcheck --nopresol
rm results/SelectedResults.csv
